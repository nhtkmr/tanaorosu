...
try {
    ...
    $ws.Cells.Font.Name = $FONT
    $ws.Cells.Font.Size = 10
    $ws.Range('A1').Value2 = '親子関係とポインター位置 ── X/Y/W/H は画像に対する 0〜1 の割合。空欄なら「位置未設定」（子一覧にだけ出ます）。塗りの薄さは空欄なら［設定］の既定値。'
    $ws.Range('A1').Font.Bold = $true

    $lhdr = @('リンクID', '親ノードID', '子ノードID', 'ラベル', 'X', 'Y', 'W', 'H', '形状', '数量', '備考', '塗りの薄さ')
    ...
    $lo = $ws.ListObjects.Add($xlSrcRange, $ws.Range('A2:L' + (2 + $links.Count)), $null, $xlYes)
    $lo.Name = 'tblLink'
    $lo.TableStyle = 'TableStyleLight10'

    $w = @(10, 13, 13, 9, 9, 9, 9, 9, 8, 7, 24, 11)
    for ($j = 0; $j -lt $w.Count; $j++) { $ws.Columns.Item($j + 1).ColumnWidth = $w[$j] }
